...
public class Heap<T> {
	...
	public void decreaseKey(int i, Integer x) {
		/*
		 * Integer num =(Integer) theHeap.get(i); num = num - (Integer) x;
		 * System.out.println(num); assign(i,(T) num); delete(i+1); siftUp();
		 * makeHeap();
		 */
		Integer num = (Integer) theHeap.get(i);
		num = num - x;
		theHeap.set(i, (T) num);
		siftUp(i);
	...
	public boolean isHeap() {
		...
		/*
		 * int padre = 0, hijoIzq = 0, hijoDer = 0;
		 * 
		 * for (int i = 0; i < theHeap.size() - 1; i++) { //if (i != 0) //padre
		 * = (Integer) theHeap.get((i-1)/2); //else padre = (Integer)
		 * theHeap.get(i); if (2 * i +1 <= theHeap.size()) hijoIzq = (Integer)
		 * theHeap.get(2 * i +1); if (2 * i + 2 <= theHeap.size()) hijoDer =
		 * (Integer) theHeap.get(2 * i + 2); if (padre > hijoDer || padre >
		 * hijoIzq) { return false;
		 * 
		 * }
		 * 
		 * }
		...
		// Realizaremos Recorrido desde arriba ("top-down")

		for (int i = 0; i < theHeap.size() - 1; i++) {

			if (2 * i + 2 <= theHeap.size() - 1) { // Si tenemos mas hijos,
													// //comprobamos para que no
													// // se salga del array
				int elementoPadre = (Integer) theHeap.get(i);
				int elementoHD = (Integer) theHeap.get(2 * i + 2);
				int elementoHI = (Integer) theHeap.get(2 * i + 1);

				if (elementoPadre > elementoHD || elementoPadre > elementoHI)
					return false;
	...
	public void makeHeap() {
		for (int i = theHeap.size() / 2 - 1; i >= 0; i--)
			siftDown(i);

	}

	public void increaseKey(int i, Integer x) {
		/*
		 * Integer num =(Integer) theHeap.get(i); num = num + (Integer) x;
		 * //System.out.println(num); assign(i,(T) num); delete(i+1);
		 * siftDown(i);
		 */
		Integer num = (Integer) theHeap.get(i);
		num = num + x;
	...
	public void replaceKey(int i, Integer x) {
		/*
		 * assign(i,(T) x); delete(i+1); siftDown(i);
		 */
		theHeap.set(i, (T) x);
		makeHeap();
	...
	public void delete(int i) {
		decreaseKey(i, (Integer) theHeap.get(i));
		removeMin();
	}
	...
	public String branchMinSum() {
		...
		String cadena;
		int posMin = -1;// theHeap.size() / 2; //indice de la primera hoja
		Integer min = Integer.MAX_VALUE;// sumarRama (posMin);
		for (int i = theHeap.size() / 2/* +1 */; i < theHeap.size(); i++) {
			Integer sumaAux = sumarRama(i);
			if (sumaAux < min) {
				min = sumaAux;
				posMin = i;
			}
		...
		cadena = "<" + min + "> --- ";
		while (posMin > 0) {
			cadena = cadena + theHeap.get(posMin) + " ";
			posMin = (posMin - 1) / 2;
		}
		cadena = cadena + theHeap.get(0) + " ";
		return cadena;
	}

	private Integer sumarRama(int pos) {
		Integer suma = 0;
		while (pos > 0) {
			suma = suma + (Integer) theHeap.get(pos);
			pos = (pos - 1) / 2;
		}
		suma = suma + (Integer) theHeap.get(0);
		return suma;
	}
	...
	} // function siftDown con start y final

	public ArrayList<Integer> sumValuesOfBranches() {
		ArrayList<Integer> lista = new ArrayList<Integer>();
		for (int i = theHeap.size() / 2; i < theHeap.size(); i++) {
			int suma = 0;
			int parent = i;
			while (parent >= 0) {
				suma = suma + (Integer) theHeap.get(parent);
				parent = (parent - 1) >> 1; //(parent - 1) /2
			}
			lista.add(suma);
		}
