...
/**
 * This class implements the Collection interface using a red-black tree as the
 * underlying storage structure.
 * 
 * @see AVLTree
 */

public class RBTree<T> implements Collection<T>, Iterable<T>,
		BinarySearchTree<T> {
			...
			if (x == x.parent.left) {
				...
				if (siblingOfx.left.color == RBNode.BLACK
					...
					if (siblingOfx.right.color == RBNode.BLACK) {
						...
						// RBNode.RED, and its right child is RBNode.BLACK.
						// perform
	...
	/**
	 * Constructs an empty RedBlack tree. All elements inserted into the tree
	...
	public boolean add(T item) {
		...
		while (curr != NIL) {
			...
			// RBNode.RED
			if (curr.left.color == RBNode.RED && curr.right.color == RBNode.RED)
				split4Node(curr);
		...
		// create the new node
		newNode = new RBNode<T>(item, (RBNode<T>) NIL, (RBNode<T>) NIL,
				(RBNode<T>) parent, RBNode.RED);
	...
	public int pathHeight(T valor) {
		int nivel = 0, comparado = 0;
		RBNode<T> nodo = this.root;

		while (nodo != null) {
			comparado = ((Comparable<T>) nodo.nodeValue).compareTo(valor);
			if (comparado == 0)
				return nivel;
			if (comparado > 0)
				nodo = nodo.left;
