...
import org.eda1.estructurasdedatos.Stack;
...
public class BSTree<T> implements Collection<T>, Iterable<T>, Cloneable,
	...
	private int height(BSTNode<T> t) {
		if (t == null)
			return -1;
		int ni = height(t.left);
		int nd = height(t.right);
		if (ni >= nd) {
	...
	public String toStringLevel(int level) {
		String cadena = toStringLevel(root, level);
		return cadena;
	}

	public String toStringLevel(BSTNode<T> t, int level) {
		String cadena = "";
		if (t != null) {
			if (level == 0)
				cadena = cadena + t.nodeValue.toString() + " ";
			else
				cadena = cadena + toStringLevel(t.left, level - 1)
	...
	private static <T> String toStringIterativePreorder(BSTNode<T> t) {
	...
	public static <T> String toStringIterativePostorder(BSTNode<T> t) {
		Stack<BSTNode<T>> colaInicial = new ALStack<BSTNode<T>>();
		Stack<T> colaFinal = new ALStack<T>();
		BSTNode<T> ptr = t;
		String cadena = "";
		colaInicial.push(ptr);

		while (!colaInicial.isEmpty()) {
			ptr = colaInicial.pop();
			if (ptr.left != null)
				colaInicial.push(ptr.left);
			if (ptr.right != null)
				colaInicial.push(ptr.right);
			colaFinal.push(ptr.nodeValue);
		}

		while (!colaFinal.isEmpty()) {
			cadena = cadena + colaFinal.pop() + "  ";
		}
	...
	private void addBalanced(ArrayList<T> aL, int left, int right) {
		...
		add(aL.get(med));
		addBalanced(aL, left, med);
		addBalanced(aL, med + 1, right);

	}

	public BSTree<T> buildSimetricTree() {
		BSTree<T> tree = new BSTree<T>();
		tree.root = buildSimetricTree(root);
		return tree;
	...
	private BSTNode<T> buildSimetricTree(BSTNode<T> t) {
		if (t == null)
			return null;
		BSTNode<T> nodo = new BSTNode<T>(t.nodeValue, null);
		nodo.left = buildSimetricTree(t.right);
		nodo.right = buildSimetricTree(t.left);
		return nodo;

	}

	public int pathHeight(T x) {
		return pathHeight(root, x, 0);
	}

	private int pathHeight(BSTNode<T> t, T x, int nivel) {
		if (t == null)
			return -1;
		if (((Comparable<T>) t.nodeValue).compareTo(x) == 0)
			return nivel;
		if (((Comparable<T>) t.nodeValue).compareTo(x) < 0)
			return pathHeight(t.right, x, nivel + 1);
		return pathHeight(t.left, x, nivel + 1);
	}

	public void removeLeaves() {
		
		root = removeLeaves(root); // al nodo asignamos el resultado de hacer la
									// operacion sobre ese nodo.
	}
