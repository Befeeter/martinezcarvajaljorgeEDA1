...
public class AVLTree<T> implements Collection<T>, Iterable<T>,
		BinarySearchTree<T> {
	...
	/**
	 * Constructs an empty AVL tree. All elements inserted into the tree must
	...
	private AVLNode<T> copy(AVLNode<T> p) {

		if (p == null)
			return null;
		AVLNode<T> nuevo = new AVLNode<T>(p.nodeValue);
		nuevo.left = copy(p.left);
		nuevo.right = copy(p.right);
		return nuevo;
	}
	...
	// borra los elementos que esten en la collection.
	public boolean removeAll(Collection<?> c) {

		Iterator it1 = c.iterator(); // creamos un iterador de tipo collection

		while (it1.hasNext()) {
			if (!remove(it1.next())) // si no se ha borrado devuelve false.
				return false;
		}
	...
	public boolean addAll(Collection <? extends T> c) {
		Iterator it1 = c.iterator(); // Creamos el iterador de tipo collection

		while (it1.hasNext()) {
			if (!add((T) it1.next())) // si no se ha añadido devuelve false.
				return false;
		}
		return true;
	}
	...
	public boolean containsAll(Collection<?> c) {
		...
		while (it1.hasNext()) {
			if (!contains(it1.next())) // Si no contiene devuele false.
				return false;
		}
		return true;
	}
	...
	public boolean retainAll(Collection<?> c) {
		AVLTree<T> sacados = new AVLTree<T>();
		Iterator<T> it1 = iterator();
		T temp = null;

		while (it1.hasNext()) {
			temp = it1.next();
			if (c.contains(temp)) // si la collection contiene el elemento lo
									// añadimos a la lista de sacados.
				sacados.add((T) temp);

		}
		clear(); // limpiamos el arbol.
		return addAll(sacados); // introducimos todos los elementos en comun con
								// la collection.

	}
	
	public <T> T[] toArray (T[] a){
		T[]array = (T[])new Object [treeSize];
